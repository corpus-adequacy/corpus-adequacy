...
**Kernel read-back, part 1 of 4 (#197).** The envelope compares the configuration the Docker daemon stored, which is not evidence about the kernel: moby/moby#49599 shows `docker inspect` reporting a value the cgroup never received. `measurements/kernel_readback.py` is the pure half of reading the kernel's own files: it parses `memory.max`, `memory.swap.max`, `pids.max`, `cpu.max` and the open-files row of `/proc/<pid>/limits`, derives what a validated `resource-profile.v2` should read back as, and names every field that differs (`readback_mismatch:<field>`) or did not read in the kernel's own form (`readback_unreadable:<field>`); either makes the record `unverified`, and no value is ever assumed. Swap follows runc's conversion: Docker's `--memory-swap` is memory plus swap, cgroup v2 `memory.swap.max` is swap alone, so the owned profile's 4 GiB and 4 GiB read back as `0`. A pids witness counts only when the kernel's `pids.events` shows a refused fork; `max 0` is `unproved`. An open-files hit cannot be witnessed this way at all: `RLIMIT_NOFILE` has no kernel-side counter, and the candidate's own output never counts. The module reads nothing itself and sits outside every contract's execution identity, so it needs no fresh PREPARE. Part 2 reads the files while the container lives, since the cgroup is removed when init exits, and places the record in a separately versioned `execution-envelope.v3`; that part changes the identity once. Parts 3 and 4 are a witness candidate and a hosted witness run.

**Kernel read-back, part 2 of 4 (#197).** A new `contained-oci-v1` run now records `execution-envelope.v3`: the v2 envelope plus `effective.kernel`, the kernel's own view of the container's limits. The cgroup is removed when the container's init exits, so the read happens while the container is held. The container's first process is the trusted wrapper, not the candidate, and on request it waits at its very start for a release file on its own `/tmp`. Meanwhile `start -a` stays attached exactly as before, the host reads `memory.max`, `memory.swap.max`, `pids.max`, `cpu.max` and `/proc/1/limits` through `docker exec` as the container user, and then releases it. No candidate code has run at that point. The release is attempted even when a read failed, so the run completes and is judged rather than timing out in the hold; a hold that is never released ends as the unproved reason `candidate-readback-hold`. A differing or unreadable value leaves the envelope `unverified`, naming the field, and withheld. Historical v0 to v2 records read as before and never gain the field, and the external `contained-oci-v0` route's container argv and result are unchanged. `measurements/kernel_readback.py` is now in every contract's execution identity, so both hosted rails need a fresh PREPARE before their next run, and the retained Slice B evidence belongs to the identity just before this.

**Kernel read-back, part 3 of 4 (#197).** The read-back shows the kernel *held* the limits, not that one was ever reached. `measurements/pids_witness.py` is a first-party witness for `pids`, the one limit whose hits the kernel counts. Its container runs under the owned `resource-profile.v2` with the sealed flags and a fixed shell script as PID 1, never a candidate. The script holds for the limits read-back, then forks in a subshell until the kernel refuses, making twice `pids.max` attempts. dash exits at the first refused fork, so the subshell dies and PID 1 survives. PID 1 waits until every short-lived child is reaped, so the cgroup has room for `docker exec` again, writes a ready file and holds a second time. The host reads `pids.events` in that second hold, then releases it. The same runner does both holds (`run_contained(..., second_hold=...)`), and each release is attempted whatever the reads did. A `corpus-adequacy.pids-witness.v0` record is `witnessed` only when all of these hold: the kernel's `max` counter is above zero, the limits read back as requested, the run completed with exit 0, the create raised no warning, and the container was removed. Anything else is `unproved`, with the reasons named. The payload's own output is never read. A local run on Docker Desktop (kernel 7.0.12-linuxkit) was `witnessed` with `max 41` in 8.4 s. The witness would not establish that any candidate was bounded, CPU, memory or open-file enforcement, or escape resistance. The change to `contained_oci.py` is inside every execution identity, like part 2. Part 4 runs the witness on a hosted rail, dispatched by the owner and read back independently.

**Kernel read-back, part 4 of 4: the hosted witness route (#197).** The witness has no subject, corpus or sites, so it does not use the mutation rails' packet. Its pre-registration is the dispatch itself. `.github/workflows/pids-witness.yml` takes two inputs: the runner revision R, and the SHA-256 of the witness's execution identity at R. That identity covers every module a witness run imports, plus the workflow itself, and a test holds the list to the imports. The owner computes it locally with `python measurements/pids_witness.py identity` and dispatches from a branch whose head is R. Before any container, the run refuses unless `GITHUB_SHA` and `GITHUB_WORKFLOW_SHA` both equal R and the identity it computes over its own checkout equals the dispatched one. It then pulls the owned toolchain image by digest, records the host as the daemon reports it (kernel, cgroup version and driver, Docker version) and runs the witness once. It seals `pids-witness-attempt.v0.json` with a predicate and `SHA256SUMS`, attested through `actions/attest`, and it does so whatever the verdict. The independent readback, run by someone other than the author, has two steps. First, `gh attestation verify <file> --repo corpus-adequacy/corpus-adequacy --signer-workflow corpus-adequacy/corpus-adequacy/.github/workflows/pids-witness.yml --predicate-type https://github.com/corpus-adequacy/corpus-adequacy/attestations/pids-witness/v0` on each downloaded file shows it came from that workflow. Without `--predicate-type`, `gh` looks only for SLSA provenance and answers 404. Only that check does so: an attempt forged consistently, values and verdict alike, would pass the second step. Second, `python measurements/pids_witness.py verify --attempt <file> --runner-revision R --identity-sha256 <digest>`, run from a checkout of R. It trusts none of the attempt's own judgements. It recomputes the identity from the checkout, compares the recorded kernel values with the owned profile, and re-derives every reason and the verdict, which must equal what was recorded. It prints one line, `pids-witness-verify=pass verdict=<verdict> reasons=<reasons>`. It catches an attempt that contradicts itself, R or the owned profile, not a consistent forgery. A hosted dispatch needs the owner's explicit go. The attempt is evidence about one GitHub-hosted runner at one time, and it keeps the part 3 non-claims.
