...
**Kernel read-back, part 4 of 4: the hosted witness route (#197).** The witness has no subject, corpus or sites, so it does not use the mutation rails' packet. Its pre-registration is the dispatch itself. `.github/workflows/pids-witness.yml` takes two inputs: the runner revision R, and the SHA-256 of the witness's execution identity at R. That identity covers every module a witness run imports, plus the workflow itself, and a test holds the list to the imports. The owner computes it locally with `python measurements/pids_witness.py identity` and dispatches from a branch whose head is R. Before any container, the run refuses unless `GITHUB_SHA` and `GITHUB_WORKFLOW_SHA` both equal R and the identity it computes over its own checkout equals the dispatched one. It then pulls the owned toolchain image by digest, records the host as the daemon reports it (kernel, cgroup version and driver, Docker version) and runs the witness once. It seals `pids-witness-attempt.v0.json` with a predicate and `SHA256SUMS`, attested through `actions/attest`, and it does so whatever the verdict. The independent readback, run by someone other than the author, is `python measurements/pids_witness.py verify --attempt <file> --runner-revision R --identity-sha256 <digest>` from a checkout of R. It trusts none of the attempt's own judgements. It recomputes the identity from the checkout, compares the recorded kernel values with the owned profile, and re-derives every reason and the verdict, which must equal what was recorded. It prints one line, `pids-witness-verify=pass verdict=<verdict> reasons=<reasons>`. A hosted dispatch needs the owner's explicit go. The attempt is evidence about one GitHub-hosted runner at one time, and it keeps the part 3 non-claims.
