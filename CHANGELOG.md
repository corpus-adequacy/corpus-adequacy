...
PREPARE v2 command path (#102): `aee_checker_sealed_run.py prepare-v2` now selects the existing canonical `prepare.v2` emitter and its frozen `resource-profile.v2` policy. The ordinary `prepare` and `prepare-v1` paths retain their previous schemas and bytes. This command performs PREPARE materialization and inert probes only; it does not execute a candidate. No hosted workflow selects it, no hosted v1 rail or owned synthetic fixture is added here, and no CPU, file-descriptor, kernel, containment or publication result is claimed. A future PREPARE changes execution identity and must be generated from the exact final runner revision.
