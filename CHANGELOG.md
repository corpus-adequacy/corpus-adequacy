...
Gate hosted contained publication (#107). A `workflow_dispatch`-only hosted lane binds candidate revision, runner revision and image digest, forces `contained-oci-v0`, refuses self-hosted/local runners, credential persistence, Docker-socket mounts and credential-like env, and writes setup, effective-envelope and candidate-result as separate artifacts under concurrency, retention and size ceilings. Missing containment is unavailable/void with score_status none — never a score — and only a verified envelope with `publication_permission: permitted` may publish; rerun evidence is append-only. Intake is not execution. This is not authentication, endorsement, audit, certification, escape-proof OCI, or a third-party quality score.
