...
digest and transformations; every gate in it must be judged, and the encoder refuses `admitted` on
a route that did not run the candidate. The v0 record is unchanged.
...
before candidate execution instead of failing after execution. The inner content
digest remains distinct from the full runner and workflow revisions that bind hosted
orchestration. This changes execution identity and requires a fresh PREPARE before
another hosted measurement.
...
Author-declared rule inventory (#121): adds `corpus-adequacy.manifest.v1`, whose required closed `rules` object maps every eligible ordinary or equivalent mutation label exactly once to an authored rule row, and the read-only `--rules <report.v0> --manifest <manifest>` sibling projection `corpus-adequacy.rules.v0`. One pure index owns field, bound, identity, eligibility, completeness, ordering and count semantics; both manifest loading and projection call it. The projection uses #151's shared 4 MiB no-follow regular-file reader for both inputs and compares exact manifest bytes with `report.manifest_sha256` before parsing. Manifest v0 remains accepted unchanged and a v0 member named `rules` stays uninterpreted; its projected inventory is `null`, while a valid empty v1 inventory carries real zeroes. `report.v0`, `survivors.v0`, scores, denominators, verdicts and mutant rows are unchanged. This is author-declared traceability, not inference, rule coverage, adequacy, owner ratification, certification, endorsement, partnership, execution, remeasurement or a change to historical evidence. Flat projected rows carry `group` so `(group, id)` remains self-describing; the corrected issue contract simultaneously says rows retain their disposition key set, so this necessary extra projection key is recorded for review.

Positional manifest input bound (#151): `load_manifest` now obtains bytes through the existing no-follow regular-file reader before parsing or hashing, using its shared 4 MiB ceiling and exact-cap behavior. Oversized manifests, symlinks, FIFOs, directories and missing paths now produce controlled input errors; previously the positional path followed symlinks and materialized regular files without a byte bound. `load_manifest_bytes` remains the sole parser and still hashes the exact input bytes, so tracked manifests and report semantics are unchanged. The reader's former `projection cap` diagnostic now says `input cap`, since the same rule protects both manifest and projection inputs. This bounds one input file; it does not sandbox runners, authenticate bytes, bound candidate memory or establish corpus adequacy.

Owned dependency-free PREPARE vendor identity (#149): the closed owned measurement contract now requires the canonical empty vendor-tree digest, while the legacy AEE contract still requires a non-empty vendor tree. Materialization and PREPARE validation call one shared contract rule, and ordinary subject/corpus tree hashing continues to refuse empty trees. No CLI or manifest selector was added, no fixture dependency was fabricated, and existing AEE schemas and archived PREPARE bytes remain unchanged. The earlier hosted PREPARE refusal was not a proof result; a fresh exact-runner PREPARE is required after this fix lands.

Sealed measurement identity seam (#102): the frozen AEE pins, corpus/tree digests, mutation schedule, candidate commands and execution-path inventory now have one immutable code-owned contract that is passed through PREPARE, authorization, execution, runtime and the hosted legacy root. There is no manifest loader, environment selector or new public option for choosing a contract. Existing AEE aliases, wire schemas, historical `prepare.v0` bytes and the `contained-oci-v0` hosted selection remain unchanged. Fresh PREPARE artifacts intentionally gain the contract module in their execution-path identity and therefore must be regenerated from the final runner revision. This refactor adds no owned fixture or hosted v1 workflow and executes no candidate.

PREPARE v2 command path (#102): `aee_checker_sealed_run.py prepare-v2` now selects the existing canonical `prepare.v2` emitter and its frozen `resource-profile.v2` policy. The ordinary `prepare` and `prepare-v1` commands retain their previous schema selection and codec/policy behavior; a newly generated artifact still receives the execution commit and content digest of the current tree, so its bytes and identity move with this source change. This command performs PREPARE materialization and inert probes only; it does not execute a candidate. No hosted workflow selects it, no hosted v1 rail or owned synthetic fixture is added here, and no CPU, file-descriptor, kernel, containment or publication result is claimed. A future PREPARE changes execution identity and must be generated from the exact final runner revision.

Daemon-reported OOM kill as a named unproved reason (#102, slice C: freeze item 5 and section 4 of the 2026-09-10 research): `run_contained` now keeps `oom_killed` beside `create_warnings`, read by `observed_oom_killed` from the same inspect it already reads: `True` or `False` when the daemon reported a bool, `None` when the field is absent or not a bool. It refuses nothing there. On both the recorded and the unrecorded candidate path, one function (`candidate_result`) makes a completed run with `oom_killed` `True` unproved with the new closed reason `oom-killed-reported`, whatever the exit code (0, 1, 137 or any other), instead of reading the inner report; `corpus_adequacy.py`'s `CLOSED_UNPROVED_REASONS` gains the token, so it survives `sanitize_unproved_reason` and reaches the report as the unproved execution detail (`[oom-killed-reported]`). containerd publishes TaskOOM when the cgroup's `memory.events` `oom_kill` counter rises, which counts processes killed by any OOM killer in that cgroup, a host-wide OOM included, and moby then sets `State.OOMKilled`; so the token names a daemon observation, is never scored and never a mutant kill, and does not claim the measured process was the one killed. `False` or `None` changes nothing: exit 137 (also `docker stop`'s SIGKILL) stays `inner-exit`, and a timeout or output cap keeps its own state even with `OOMKilled` true. PID and `nofile` exhaustion stay cause-unobserved and CPU throttling is not a cause. The execution envelope is unchanged: no key is added to v0 (its keyset is frozen) or v1, so the envelope does not carry the OOM observation; its `candidate_outcome` for such a run is `unproved`, as for any unproved run, and the report's unproved reason carries it. No verdict changes other than through this token. `contained_oci.py`, the candidate and `corpus_adequacy.py` are execution paths, so a fresh PREPARE is required; none is regenerated here. The path set differs from the frozen C ceiling: `corpus_adequacy.py` and `README.md` are added because the closed reason set lives in the engine, `tests/test_aee_checker_sealed_runtime.py` is added for the detail and report-suffix tests, and `effective_envelope.py` is not touched. Leaves #102 open. The same closed set also gains `setup`: `_refused_envelope` has always asked for that reason for a run whose contained setup was unavailable or refused, but it was not a closed reason, so `sanitize_unproved_reason` turned it into `malformed`, which points at the candidate's output for a run that never reached an outcome.
