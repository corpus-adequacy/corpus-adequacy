    ...
    "jobs": {"owned-contained": {
        ...
        "steps": [
            ...
            {"name": "Upload report",
             "if": "steps.gate.outcome == 'success' && !cancelled()",
             "uses": UPLOAD_ACTION,
             "with": {"name": "owned-contained-v1-report",
                      "path": "owned-contained-v1-artifacts/report.v0.json",
                      "retention-days": 14, "if-no-files-found": "error"}},
...
class OwnedHostedRailContract(unittest.TestCase):
    ...
    def test_new_workflows_are_disjoint_and_legacy_bytes_stay_fixed(self):
        expected = {
            ...
            # Moved once for #187 (seal and attest steps); the owned workflows stay disjoint.
            ".github/workflows/contained-hosted-publication.yml":
                "0a9b79ccb551bb3cf8e64122f28f7a667da351ebf1568458f8b815db351c15a9",
        }
